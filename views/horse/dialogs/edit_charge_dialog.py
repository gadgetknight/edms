...
Purpose: Dialog for editing the details of a single charge transaction.
         - Applied standard "boxed" and colored style to Save/Cancel buttons.
...
- v1.1.0 (2025-06-07):
    - Applied a new, standardized style to all input widgets to create a
      consistent "boxed-in" look (white border, dark gray background).
- v1.0.3 (2025-06-07):
    - Bug Fix: Corrected all references to the transaction notes field from
      `.notes` to the correct model attribute, `.item_notes`.
- v1.0.2 (2025-06-07):
    - Bug Fix: Corrected all references to the transaction notes field from
      `.notes` to the correct model attribute, `.item_notes`.
- v1.0.1 (2025-06-06):
...
class EditChargeDialog(QDialog):
    ...
    def _apply_styles(self):
        ...
        self.service_date_edit.setStyleSheet(field_style)
        self.description_edit.setStyleSheet(field_style)
        self.qty_spinbox.setStyleSheet(field_style)
        self.price_spinbox.setStyleSheet(field_style)
        self.taxable_checkbox.setStyleSheet(field_style)
