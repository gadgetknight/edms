...
Purpose: Provides a tabbed interface for managing system settings.
         Corrects TypeError in load_master_owners_data related to setEnabled.
Last Updated: May 21, 2025
Author: Claude Assistant (Modified by Gemini)

Changelog:
- v1.10.7.2 (2025-05-21):
    - Removed incorrect self.delete_master_owner_btn.setEnabled() call from
      load_master_owners_data to fix TypeError. Button state is handled by
      update_owner_form_button_states via selection change.
- v1.10.7.1 (2025-05-21):
    - Added prominent log at the start of setup_ui.
    - Corrected call from get_user_by_id to get_user_by_login_id in on_user_selection_changed.
    - Removed 'owner_id_to_check_for_unique' from validate_owner_data call in
      handle_save_master_owner_changes to match OwnerController v1.3.2.
- v1.10.7 (2025-05-19):
    - Changed import for AddEditChargeCodeDialog to reflect user's preferred
      filename: '.dialogs.add_edit_change_code_dialog'.
# ... (rest of original changelog for v1.10.7)
"""
...
from PySide6.QtWidgets import (
    ...
    QAbstractItemView,
    QSizePolicy,
    QStatusBar,  # Keep even if not directly used by this class for BaseView
)
...
# Dialog imports from your v1.10.7 structure
...
class AddMasterOwnerDialog(QDialog):
    def __init__(self, parent_controller, parent=None):
        ...
        dialog_fields = [
            ...
            ("Country:", "country_name", "QLineEdit", "e.g. USA"),  # Display only
    ...
    def get_data(self) -> dict:
        ...
        # country_name is not part of Owner model for saving, it's derived
...
class UserManagementScreen(BaseView):
    ...
    def __init__(self, current_user_id: str):
        ...
        self.owners_tab: Optional[QWidget] = None
        self.charge_codes_tab: Optional[QWidget] = None
        self.locations_tab: Optional[QWidget] = None
        self.user_list_widget: Optional[UserListWidget] = None
        self.owner_master_list_widget: Optional[OwnerMasterListWidget] = None
        ...
        self.status_label_footer: Optional[QLabel] = None
        self.detail_user_id_input: Optional[QLineEdit] = None
        self.detail_user_name_input: Optional[QLineEdit] = None
        self.detail_role_combo: Optional[QComboBox] = None
        self.detail_is_active_checkbox: Optional[QCheckBox] = None
        self.save_user_changes_btn: Optional[QPushButton] = None
        self.change_password_btn_form: Optional[QPushButton] = None
        self.delete_user_btn: Optional[QPushButton] = None
        self.add_user_btn: Optional[QPushButton] = None
        self.user_details_form_widget: Optional[QFrame] = None
        self.user_details_panel_placeholder: Optional[QLabel] = None
        self.add_master_owner_btn: Optional[QPushButton] = None
        self.delete_master_owner_btn: Optional[QPushButton] = None
        self.owner_details_form_widget: Optional[QFrame] = None
        self.owner_details_panel_placeholder: Optional[QLabel] = None
        self.save_master_owner_changes_btn: Optional[QPushButton] = None
        ...
        self.horse_mgmt_btn_header: Optional[QPushButton] = None
        ...
        self.logger.info(
            "UserManagementScreen initialized with Tabbed Interface (v1.10.7.2)."
        )
    ...
    def setup_ui(self):
        ...
        self.set_title("System Setup & Administration (v1.10.7.2)")
        self.resize(1000, 700)

        if self.central_widget.layout() is None:
            self.logger.info(
                "UserManagementScreen.setup_ui: central_widget has no layout, creating new QVBoxLayout."
            )
            main_layout = QVBoxLayout(self.central_widget)
        else:
            self.logger.info(
                f"UserManagementScreen.setup_ui: central_widget already has layout: {type(self.central_widget.layout())}. Using existing."
            )
            main_layout = self.central_widget.layout()
            if not isinstance(main_layout, QVBoxLayout):
                self.logger.warning(
                    "UserManagementScreen.setup_ui: Existing layout is not QVBoxLayout. Re-creating as QVBoxLayout."
                )
    ...
    def setup_users_tab_ui(self, parent_tab_widget):
        ...
        # Use AddEditUserDialog.USER_ROLES if it's defined there, or define locally
        # For now, assuming AddEditUserDialog (imported) has this static member.
        if hasattr(AddEditUserDialog, "USER_ROLES"):
            self.detail_role_combo.addItems(AddEditUserDialog.USER_ROLES)
        else:  # Fallback if USER_ROLES is not on AddEditUserDialog
            self.logger.warning(
                "AddEditUserDialog.USER_ROLES not found, using hardcoded roles for UserManagementScreen."
            )
            self.detail_role_combo.addItems(["admin", "vet", "staff", "owner"])

        self.detail_role_combo.setStyleSheet(specific_input_style_form)
        self.detail_role_combo.currentIndexChanged.connect(
            self.on_user_detail_field_changed
        )
        role_label = QLabel("Role*:")
        role_label.setStyleSheet(label_style_form)
        details_form_layout.addRow(role_label, self.detail_role_combo)
    ...
    def setup_locations_tab_ui(self, parent_tab_widget):
        ...
        self.locations_table.setColumnWidth(1, 250)
    ...
    def setup_connections(self):
        ...
        if hasattr(self, "charge_codes_table") and self.charge_codes_table:
            self.charge_codes_table.itemSelectionChanged.connect(
                self.update_charge_code_buttons_state
            )

        if hasattr(self, "add_location_btn") and self.add_location_btn:
            self.add_location_btn.clicked.connect(self.handle_add_location)
        if hasattr(self, "edit_location_btn") and self.edit_location_btn:
            self.edit_location_btn.clicked.connect(self.handle_edit_location)
        if (
            hasattr(self, "toggle_location_active_btn")
            and self.toggle_location_active_btn
        ):
            self.toggle_location_active_btn.clicked.connect(
                self.handle_toggle_location_active
            )
        if hasattr(self, "locations_table") and self.locations_table:
    ...
    def load_users_data(self):
        ...
        if not self.user_list_widget:
            return  # Guard if UI not fully setup
        ...
        if not self.users_list_data:
            ...
            if hasattr(self, "delete_user_btn") and self.delete_user_btn:
                self.delete_user_btn.setEnabled(False)
            return
        ...
        for i, user_obj in enumerate(self.users_list_data):
            ...
            item_text = f"{user_obj.user_id} - {user_obj.user_name} ({role_name}) ({'Active' if user_obj.is_active else 'Inactive'})"
    ...
    def load_master_owners_data(self):
        ...
        if not self.owner_master_list_widget:
            return  # Guard
        ...
        if not self.owners_master_list_data:
            ...
            # Removed the direct setEnabled call that caused TypeError
            return
        ...
        if new_selection_index != -1:
            ...
            self.display_owner_details_form(False)
        # Button states will be updated by on_master_owner_selection_changed if a row is selected
        ...
        try:
            ...
            for row_idx, loc in enumerate(locations):
                ...
                address_parts = [
                    loc.address_line1,
                    loc.city,
                    loc.state.state_code if loc.state else loc.state_code,
                    loc.zip_code,
                ]
                full_address = ", ".join(filter(None, address_parts))
                description_display = loc.description or full_address or "N/A"

                self.locations_table.setItem(
                    row_idx, 2, QTableWidgetItem(description_display)
                )
    ...
    def on_user_selection_changed(self):
        ...
        if selected_items:
            ...
            self.current_selected_user_obj = self.user_controller.get_user_by_login_id(
    ...
    def update_charge_code_buttons_state(self):
        if (
            not self.charge_codes_table
            or not self.edit_charge_code_btn
            or not self.toggle_charge_code_active_btn
        ):
            return
    ...
    def update_location_buttons_state(self):
        if (
            not self.locations_table
            or not self.edit_location_btn
            or not self.toggle_location_active_btn
        ):
            return
    ...
    def display_user_details_form(self, show: bool):
        if not self.user_details_form_widget or not self.user_details_panel_placeholder:
            return
        if show:
            ...
            if hasattr(self, "detail_user_id_input") and self.detail_user_id_input:
                self.detail_user_id_input.clear()
            if hasattr(self, "detail_user_name_input") and self.detail_user_name_input:
                self.detail_user_name_input.clear()
            if hasattr(self, "detail_role_combo") and self.detail_role_combo:
                self.detail_role_combo.setCurrentIndex(0)
            if (
                hasattr(self, "detail_is_active_checkbox")
                and self.detail_is_active_checkbox
            ):
    ...
    def populate_user_details_form(self, user: User):
        if not self.detail_user_id_input:
            return  # Guard
        self.detail_user_id_input.setText(user.user_id)
        self.detail_user_name_input.setText(user.user_name or "")

        if hasattr(self, "detail_role_combo") and self.detail_role_combo:
            # Fallback roles if AddEditUserDialog.USER_ROLES is not accessible or defined yet
            defined_roles = ["admin", "vet", "staff", "owner"]
            if hasattr(AddEditUserDialog, "USER_ROLES"):
                defined_roles = AddEditUserDialog.USER_ROLES

            role_name_to_select = user.roles[0].name if user.roles else defined_roles[0]
            role_index = self.detail_role_combo.findText(
                role_name_to_select, Qt.MatchFlag.MatchExactly
            )
            self.detail_role_combo.setCurrentIndex(role_index if role_index >= 0 else 0)
    ...
    def display_owner_details_form(self, show: bool):
        if (
            not self.owner_details_form_widget
            or not self.owner_details_panel_placeholder
        ):
            return
    ...
    def handle_add_user(self):
        ...
        dialog = AddEditUserDialog(self, self.user_controller, current_user_object=None)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self.show_info("User Action", "New user processed by dialog.")
            self.load_users_data()
        else:
    ...
    def handle_save_user_changes(self):
        ...
        user_id_to_update = self.current_selected_user_obj.user_id
        self.logger.info(f"Saving user: {user_id_to_update}")
        ...
        success, message = self.user_controller.update_user(
            user_id_to_update, updated_data, self.current_user_id
        )
        if success:
    ...
    def handle_delete_user(self):
        ...
        if reply:
            ...
            if hasattr(self.user_controller, "delete_user_permanently"):
                success, message = self.user_controller.delete_user_permanently(
                    user_id_to_delete, self.current_user_id
                )
            else:
                self.logger.warning(
                    f"UserController.delete_user_permanently not found. Attempting to deactivate user {user_id_to_delete} instead."
                )
                updated_data = {"is_active": False}
                success, message = self.user_controller.update_user(
                    user_id_to_delete, updated_data, self.current_user_id
                )
                if success:
                    message = f"User '{user_name_display}' deactivated as delete is not fully implemented."

            if success:
                self.show_info("User Action", message)
                self.selected_user_id = None
                self.current_selected_user_obj = None
                self.load_users_data()
            else:
                self.show_error("Action Failed", message)
        else:
    ...
    def handle_change_password(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            success, message = self.user_controller.change_password(
                self.selected_user_id, new_password, self.current_user_id
            )
            if success:
    ...
    def handle_save_master_owner_changes(self):
        ...
        owner_id_to_update = self.current_selected_master_owner_obj.owner_id
        self.logger.info(f"Saving changes for master owner: {owner_id_to_update}")
        ...
        is_valid, errors = self.owner_controller.validate_owner_data(
            owner_data_to_save, is_new=False
        )
        if not is_valid:
            self.show_error(
        ...
        if success:
            ...
            self.load_master_owners_data()
            if self.selected_master_owner_id:
                for i in range(self.owner_master_list_widget.count()):
                    if (
                        self.owner_master_list_widget.item(i).data(
                            Qt.ItemDataRole.UserRole
                        )
                        == self.selected_master_owner_id
                    ):
                        self.owner_master_list_widget.setCurrentRow(i)
                        break
            self.has_owner_details_changed = False
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            QTimer.singleShot(
                ...
                lambda: (
                    ...
                    if self.status_label_footer
                    and self.status_label_footer.text()
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            QTimer.singleShot(
                ...
                lambda: (
                    self.status_label_footer.setText("Ready")
                    if self.status_label_footer
                    and self.status_label_footer.text() == msg
                    else None
                ),
            )
            ...
            if success:
                ...
                QTimer.singleShot(
                    ...
                    lambda: (
                        self.status_label_footer.setText("Ready")
                        if self.status_label_footer
                        and self.status_label_footer.text() == message
                        else None
                    ),
                )
            else:
    ...
    def handle_toggle_location_active(self):
