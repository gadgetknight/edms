...
Changelog:
- v1.1.1 (2025-06-28):
    - **BUG FIX**: Modified `validate_veterinarian_data` to explicitly check if `email`
      is `None` before attempting to call `.strip()`, resolving `AttributeError`.
- v1.1.0 (2025-06-09):
...
class VeterinarianController:
    ...
    def validate_veterinarian_data(
        ...
        # BUG FIX: Handle None explicitly before calling .strip()
        email_value = vet_data.get("email")
        email = (
            email_value.strip()
        ...
        try:
            ...
            if (
                email
            ):  # Only validate and check uniqueness if email is provided (not None or empty string)
                if not re.match(
