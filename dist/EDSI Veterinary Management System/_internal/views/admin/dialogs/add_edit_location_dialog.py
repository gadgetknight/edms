...
- v1.1.7 (2025-06-28):
    - **BUG FIX**: Corrected `db_manager.get_session()` to `db_manager().get_session()`
      in `_load_states_into_combobox` to correctly retrieve the `DatabaseManager`
...
class AddEditLocationDialog(QDialog):
    ...
    def _get_form_input_style(self, base_bg=DARK_INPUT_FIELD_BACKGROUND) -> str:
        return f"""
            ...
            QLineEdit:disabled, QComboBox:disabled {{ 
                ...
                border-color: {AppConfig.DARK_HEADER_FOOTER};
            }}
            QComboBox::drop-down {{ 
                border: none;
    ...
    def _setup_palette(self):
        ...
        palette.setColor(QPalette.ColorRole.Link, QColor(AppConfig.DARK_PRIMARY_ACTION))
        palette.setColor(
    ...
    def _create_label(self, text: str) -> QLabel:
        label = QLabel(text)
        label.setStyleSheet(
            f"color: {AppConfig.DARK_TEXT_PRIMARY}; background-color: transparent; padding-top: 3px; font-size: 13px;"
        )
        font_size = (
        ...
        label.setFont(QFont(AppConfig.DEFAULT_FONT_FAMILY, font_size))
        return label
    ...
    def _setup_ui(self):
        ...
        self.is_active_checkbox.setStyleSheet(
            f"color: {AppConfig.DARK_TEXT_PRIMARY}; font-size: 13px;"
        )
    ...
    def _load_states_into_combobox(self):
        ...
        try:
            ...
            # CORRECTED: Call db_manager() to get the instance before calling .get_session()
            ...
            if session:
                # CORRECTED: Call db_manager().close() to close the session from the manager
                db_manager().close()
