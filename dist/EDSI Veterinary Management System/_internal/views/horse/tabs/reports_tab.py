...
Changelog:
- v1.9.0 (2025-06-29):
    - Modified all report generation methods (`_run_owner_statement_report`, `_run_ar_aging_report`,
      `_run_invoice_register_report`, `_run_payment_history_report`, `_run_charge_code_usage_report`,
      `_run_horse_transaction_history_report`) to use `AppConfig.get_accounting_reports_dir()`
      as the default save location for PDF files.
    - Adjusted `_generate_single_statement` to also use `AppConfig.get_accounting_reports_dir()`
      for owner statements, ensuring consistency.
- v1.8.0 (2025-06-12):
...
class ReportsTab(QWidget):
    ...
    def _run_charge_code_usage_report(self):
        ...
        # MODIFIED: Use AppConfig.get_accounting_reports_dir()
        default_path = os.path.join(
            AppConfig.get_accounting_reports_dir(), default_filename
        )
    ...
    def _generate_batch_statements(self, start_date: date, end_date: date):
        ...
        save_dir = QFileDialog.getExistingDirectory(
            self,
            "Select Directory to Save Statements",
            AppConfig.get_accounting_reports_dir(),
        )
        if not save_dir:
            return
    ...
    def _generate_single_statement(
        ...
        # MODIFIED: Use AppConfig.get_accounting_reports_dir()
        save_dir = AppConfig.get_accounting_reports_dir()
